...
BASE_BB_VER="BoomBox"
VER="V4"
BB_VER="$BASE_BB_VER$VER"

# Vars
export LOCALVERSION="-$BB_VER-$(date +%Y%m%d)"
export CROSS_COMPILE="/home/amardeep434/toolchains/arm-eabi-5.3/bin/arm-eabi-"
export ARCH=arm
export SUBARCH=arm
...
FINAL_ZIP="/home/amardeep434/android/kernel/$BB_VER-$(date +%Y%m%d).zip"
# Functions
...
function make_zip {
		cd $REPACK_DIR
		zip -r9 ~/android/kernel/$BB_VER-$(date +%Y%m%d).zip *
		cd $KERNEL_DIR
